...
# ================= 6. UI 版面佈局 =================
# 【絕殺解法】：全部使用強制像素大小的 div，不給 Streamlit 任何偷偷縮放的機會
st.markdown("<div style='font-size: 28px; font-weight: 900; margin-bottom: 15px;'>🏫 正德調課小幫手</div>", unsafe_allow_html=True)
...
with tab_swap:
    if my_name:
        ...
        with col_c1:
            st.markdown(f"<div style='font-size: 28px; font-weight: 900; margin-bottom: 15px;'>📅 【{my_name}老師】的課表</div>", unsafe_allow_html=True)
        ...
        with col_c2:
            if st.session_state.uni_target_tb and st.session_state.uni_target_tb in all_swaps:
                ...
                if opt['direct']:
                    st.markdown(f"<div style='font-size: 22px; font-weight: 900; margin-bottom: 10px;'>👀 {opt['Teacher_B']}老師的課表變化</div>", unsafe_allow_html=True)
                    ...
                    with st.container(border=True):
                        st.markdown("<div style='font-size: 18px; font-weight: 900; margin-bottom: 10px;'>📥 確認無誤，加入列印清單</div>", unsafe_allow_html=True)
                    ...
                    st.markdown("<div style='font-size: 22px; font-weight: 900; margin-bottom: 10px;'>💡 請選擇協助的［橋樑］老師</div>", unsafe_allow_html=True)
                    bridge_options = {}
                    for c in opt['chains']:
                    ...
                    st.markdown(f"<div style='font-size: 22px; font-weight: 900; margin-top: 15px; margin-bottom: 10px;'>👀 {opt['Teacher_B']}老師的課表變化</div>", unsafe_allow_html=True)
                    try: st.dataframe(grid_b.style.map(style_target_grid), use_container_width=True, height=320)
                    except AttributeError: st.dataframe(grid_b.style.applymap(style_target_grid), use_container_width=True, height=320)

                    st.markdown(f"<div style='font-size: 22px; font-weight: 900; margin-top: 15px; margin-bottom: 10px;'>👀 {c_data['Teacher_C']}老師的課表變化</div>", unsafe_allow_html=True)
...
with tab_print:
    with st.container(border=True):
        st.markdown("<div style='font-size: 22px; font-weight: 900; margin-bottom: 10px;'>⚙️ 單據表頭設定</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div style='font-size: 22px; font-weight: 900; margin-top: 15px; margin-bottom: 10px;'>📝 待列印清單編輯區</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div style='font-size: 22px; font-weight: 900; margin-top: 15px; margin-bottom: 10px;'>🖨️ 列印與輸出</div>", unsafe_allow_html=True)
    with st.container(border=True):
